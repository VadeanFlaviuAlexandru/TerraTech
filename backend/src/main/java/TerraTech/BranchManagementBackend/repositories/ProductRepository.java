...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("""
                ...
                FROM (SELECT 1 as MONTH UNION SELECT 2 UNION SELECT 3
                      UNION SELECT 4 UNION SELECT 5 UNION SELECT 6
                      UNION SELECT 7 UNION SELECT 8 UNION SELECT 9
                      UNION SELECT 10 UNION SELECT 11 UNION SELECT 12) MONTHS
                CROSS JOIN Product p
                LEFT JOIN p.reports r
                       ON MONTH(r.createDate) = MONTHS.MONTH AND YEAR(r.createDate) = :year AND p.id = :id
